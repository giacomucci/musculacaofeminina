...
class Mage_Sales_Model_Order_Api extends Mage_Sales_Model_Api_Resource
{
    ...
    public function info($orderIncrementId)
    {
        ...
        $result['vlr_frete'] = $result['shipping_amount']; //Envia valor do frete para o Bling
        $result['vlr_desconto'] = $result['discount_amount']; //Envia valor do desconto para o Bling
        if(!$result['customer_taxvat']){
                $result['customer_taxvat'] = $result['shipping_address']['cpfcnpj']; //Enviar o CPF ou CNPJ para o Bling
        }
        $result['transporte']['transportadora'] = "correios"; //Envia o nome da transportadora
        
        //Envia o tipo de envio ¼
        if($result['shipping_method'] == "04162"){
                $result['transporte']['tipo_frete'] = "D"; //Frete por conta do destinatário
                $result['transporte']['servico_correios'] = "SEDEX";
                $result['shipping_method'] =  "04162";
                $result['shipping_description'] ="04162";
        }
        if($result['shipping_method'] == "04669"){
                $result['transporte']['tipo_frete'] = "D";
                $result['transporte']['servico_correios'] = "PAC";
                $result['shipping_method'] = "04669";
                $result['shipping_description'] ="04669";
        }
        //fim do código personalizado
        
        Mage::log(var_dump($result));
