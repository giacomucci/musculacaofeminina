    ...
    */
    ...
    public function newsletterSubscribe($event)
    {
        ...
        if (!$subscriber->getCustomerId()) {
            $this->validateCandidadeToVisitorsTable($subscriber->getData($emailKey));
        }
        ...
        if ($this->isNeedToProcessSubscriptionRuleQueue($subscriber, $statusKey, $statusSubscribed)) {//process rule queue if subscription status is active
            $ruleIds = Mage::getModel('followupemail/mysql4_rule')
    ...
    public function salesQuoteSaveAfter($observer)
    {
        if ($quote = $observer->getEvent()->getQuote()) {
            if ($quote->getCustomerEmail() || $quote->getCustomerId()) {
                return $this;
            }

            $customerSession = Mage::getSingleton('customer/session');
            $visitorsemailModel = Mage::getModel('followupemail/visitorsemail');
            $visitor = $visitorsemailModel->loadByQuoteId($quote->getId());
            if (!$visitor) {
                $visitor = $visitorsemailModel;
                $data = array(
                ...
                $visitor->setQuoteUpdatedAt($quote->getUpdatedAt());
            }
            try {
                $visitor->save();
            } catch (Exception $e) {
    ...
    {
        if (
            Mage::app()->getRequest()->getModuleName() == 'contacts'
            && Mage::app()->getRequest()->getActionName() == 'post'
        ) {
            ...
            if (isset($post['email'])) {
                ...
                if (!$customer->getId()) {
                    $this->validateCandidadeToVisitorsTable($post['email']);
                }
            }
        }
    }

    private function validateCandidadeToVisitorsTable($email)
    {
        $customerSession = Mage::getSingleton('customer/session');
        $visitorsemailModel = Mage::getModel('followupemail/visitorsemail');
        $visitor = $visitorsemailModel->loadBySessionId($customerSession->getSessionId());
        if ($visitor) {
